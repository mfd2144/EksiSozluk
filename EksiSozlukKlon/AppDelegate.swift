...
class AppDelegate: UIResponder, UIApplicationDelegate,GIDSignInDelegate {
 ...
 func application( _ app: UIApplication, open url: URL, options: [UIApplication.OpenURLOptionsKey : Any] = [:] ) -> Bool {
    ...
    func sign(_ signIn: GIDSignIn!, didSignInFor user: GIDGoogleUser!, withError error: Error?) {
        ...
        if let control = GIDSignIn.sharedInstance()?.presentingViewController as? LoginViewController {
            control.model.userLogin(credential)
        }else if let control = GIDSignIn.sharedInstance()?.presentingViewController as? LogupViewController {
            control.model.userLogin(credential)
        }
    }
